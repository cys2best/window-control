...
export function Login({ navigation }: { navigation: any }) {
  ...
  const submit = async () => {
    ...
    const redirectTo = effectiveBase ? `${effectiveBase}/login` : undefined;
    const authFn = mode === "sign-in" ? signInWithPassword : signUpWithPassword;
    const result = await authFn(supabaseUrl, supabaseAnonKey, email, password, redirectTo);
    setBusy(false);
    if ("error" in result) {
  ...
  return (
    <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === "ios" ? "padding" : undefined}>
      <View style={{ flex: 1, backgroundColor: theme.color.screen, padding: 24, paddingTop: 80 }}>
        ...
        <TextInput value={email} onChangeText={(t) => { setEmail(t); setError(""); setNotice(""); }}
        ...
        <TextInput value={password} onChangeText={(t) => { setPassword(t); setError(""); setNotice(""); }}
        ...
        {notice ? (
          <Text style={{ fontFamily: theme.font.semibold, fontSize: 13, color: "#10b981", marginTop: 10 }}>
            {notice}
          </Text>
        ) : null}
        ...
        <Text
          onPress={() => { setMode(mode === "sign-in" ? "sign-up" : "sign-in"); setError(""); setNotice(""); }}
