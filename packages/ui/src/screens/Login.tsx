  ...
  return (
    <KeyboardAvoidingView style={{ flex: 1, backgroundColor: theme.color.screen }} behavior={Platform.OS === "ios" ? "padding" : undefined}>
      <ScrollView keyboardShouldPersistTaps="handled" contentContainerStyle={{ flexGrow: 1, padding: 24, paddingTop: 64 }}>
        <BrandMark />
        <Text style={{ fontFamily: theme.font.bold, fontSize: 27, color: theme.color.text, marginTop: 24, marginBottom: 16 }}>
        ...
        <View style={{ flexDirection: "row", padding: 4, backgroundColor: theme.color.surfaceRaised, borderRadius: theme.radius.pill, marginBottom: 24 }}>
          {(["sign-in", "sign-up"] as const).map((value) => (
            <Pressable key={value} accessibilityRole="tab" accessibilityState={{ selected: mode === value, disabled: busy }}
              ...
              <Text style={{ fontFamily: theme.font.monoMedium, fontSize: 11, color: mode === value ? theme.color.bg : theme.color.textMuted }}>
        ...
        {mode === "sign-up" ? (
          <TextInput value={pairingCode} onChangeText={(value) => { setPairingCode(value); clearFeedback(); }}
            ...
            style={{ ...fieldStyle("pairing"), ...inputStyle, fontFamily: theme.font.mono, letterSpacing: 1.2, marginTop: 12 }} />
        ) : null}
        <View style={{ marginTop: 20, padding: 14, backgroundColor: theme.color.surfaceRaised, borderLeftWidth: 2, borderLeftColor: theme.color.live }}>
