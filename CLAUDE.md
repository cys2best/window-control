...
- `apps/desktop` — pywebview desktop shell (`tray.py`, `window.py`; relocated from `src/gui/tray.py`) embedding `apps/web`'s build in a native window alongside the existing PyQt5 tray/launcher. Test: `uv run pytest apps/desktop/ -v` (part of `src`'s Python toolchain, not a separate npm workspace).
